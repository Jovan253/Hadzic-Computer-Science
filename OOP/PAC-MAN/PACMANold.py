...
while not done:
    ...
    if Blinky_pos in node_list:
        if pacman.rect.x >= Blinky.rect.x:
            #blinky needs to go right
            #need to check if he can, at this 'y height'
            if (Blinky.rect.x + 30) in blocked_x and Blinky.rect.y in blocked_y:
                #blinky is blocked -- he cannot go right, so no horizontal movement
                Blinky.horizontal(0)
            else:
                Blinky.horizontal(1)
        else:
            #blinky needs to go left
            #need to check if he can, at this 'y height'
            if (Blinky.rect.x - 30) in blocked_x and Blinky.rect.y in blocked_y:
                #blinky is blocked -- he cannot go left, so no horizontal movement
                Blinky.horizontal(0)
            else:
                Blinky.horizontal(-1)
...
'''
    ...
    if Blinky_pos in Node_List:
        if pacman.rect.x > Blinky.rect.x:
            ...
            #if (Blinky.rect.x + 30) in blocked_x:
             #   Blinky.horizontal(-1)
              #  Blinky.vertical(0)
            #elif (Blinky.rect.y - 30) in blocked_y:
             #   Blinky.vertical(1)
              #  Blinky.horizontal(0)
            #else:                
            Blinky.vertical(-1)
