...
while not done:
    ...
    for junction in junction_list:
        ...
        if BlinkyAtJunction:
            ...
            print("Blinky_blocked = ", Blinky_blocked)
            print("Blinky moving = ", Blinky_moving)

            if (Blinky_blocked == "UP" or Blinky_blocked == "DOWN"):   
                Blinky.vertical(0)
                if pacman.rect.x == Blinky.rect.x:
                    ...
                    if junction.canGoRight:
                        Blinky.horizontal(1)
                        Blinky_moving = "RIGHT"
                        print("moving right ", Blinky.rect.x, Blinky.rect.y)
                    else:
                        print("blocked right at = ", junction.x, junction.y)
                        Blinky.horizontal(0)
                        Blinky_blocked = "RIGHT"
                else:
                    ...
                    if junction.canGoLeft:
                        Blinky.horizontal(-1)
                        Blinky_moving = "LEFT"
                        print("moving left", Blinky.rect.x, Blinky.rect.y)
                    else:
                        print("blocked left at = ", junction.x, junction.y)
                        Blinky.horizontal(0)
                        Blinky_blocked = "LEFT"

                
            if (Blinky_blocked == "LEFT" or Blinky_blocked == "RIGHT"):   
                Blinky.horizontal(0)
                if pacman.rect.y == Blinky.rect.y:
                    print("pacman.y = blinky.y at ", Blinky.rect.x, Blinky.rect.y)
                    if junction.canGoLeft and pacman.rect.x <= Blinky.rect.x:
                        Blinky.horizontal(-1)
                        Blinky.vertical(0)
                        Blinky_moving = "LEFT"
                        print("moving left", Blinky.rect.x, Blinky.rect.y)
                    elif junction.canGoRight and pacman.rect.x >= Blinky.rect.x:
                        Blinky.horizontal(1)
                        Blinky.vertical(0)
                        Blinky_moving = "RIGHT"
                        print("moving right", Blinky.rect.x, Blinky.rect.y)
                        ...
                        Blinky.vertical(-1)
                        Blinky_moving = "UP"
                        print("moving up", Blinky.rect.x, Blinky.rect.y)
                    elif junction.canGoDown:
                        Blinky.vertical(1)
                        Blinky_moving = "DOWN"
                        print("moving down", Blinky.rect.x, Blinky.rect.y)
                    else:
                        print("BROKEN!! ", Blinky.rect.x, Blinky.rect.y)

                elif pacman.rect.y < Blinky.rect.y:
                    print("blinky to go up = ", Blinky.rect.x, Blinky.rect.y)
                    if junction.canGoUp:
                        Blinky.vertical(-1)
                        Blinky_moving = "UP"
                        print("moving up ", Blinky.rect.x, Blinky.rect.y)
                    else:
                        print("blocked up at = ", junction.x, junction.y)
                        Blinky.vertical(0)
                        Blinky_blocked = "UP"
                else:
                    print("blinky to go down = ", Blinky.rect.x, Blinky.rect.y)
                    if junction.canGoDown:
                        Blinky.vertical(1)
                        Blinky_moving = "DOWN"
                        print("moving down ", Blinky.rect.x, Blinky.rect.y)
                    else:
                        print("blocked down at = ", junction.x, junction.y)
                        Blinky.vertical(0)
                        Blinky_blocked = "DOWN"
